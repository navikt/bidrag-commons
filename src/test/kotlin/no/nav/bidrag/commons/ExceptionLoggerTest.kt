...
//    every { appenderMock.doAppend(any()) } just runs
  ...
  fun `skal returnere det som logges`() {
    val exceptionStreng = java.lang.String.join(
      "", exceptionLogger.logException(
        HttpClientErrorException(HttpStatus.BAD_REQUEST, "oops", "something is fishy".toByteArray(), null), "junit test"
      )
    )
  ...
  fun `skal logge en error per exception`() {
    exceptionLogger.logException(
      HttpClientErrorException(HttpStatus.BAD_REQUEST, "oops", "i did it again".toByteArray(), null), "junit test"
    )
